    ...
    IJobExecuter jobExecuter) : IOutboxJobScheduler
{
    ...
    {
        if (outboxMessage.DeliveryLeaseId is not null)
            return await jobExecuter.EnqueueAsync<IOutboxExecutionJob>((job, ct) => job.Run(outboxMessage.Id, ct), OutboxQueue, cancellationToken);

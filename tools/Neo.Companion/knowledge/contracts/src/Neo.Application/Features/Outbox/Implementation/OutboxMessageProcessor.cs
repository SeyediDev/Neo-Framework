    ...
    where TOutboxMessage : IOutboxMessage
{
        ...
        TOutboxMessage message, string idempotencyKey, string tenantKey, CancellationToken ct)
    {
        ...
        if (existing != null)
        {
            ...
            // A missing row may be a visibility/replication issue. Never delete another writer's key blindly.
            throw new InvalidOperationException("Idempotency record refers to an unavailable outbox row; reconcile it before retrying.");
        }
        ...
        TOutboxMessage message, string? idempotencyKey, string? tenantKey, CancellationToken ct)
    {
        var outboxMessage = new OutboxMessage
        {
			...
			TenantKey = tenantKey,
			OutboxState = string.IsNullOrEmpty(idempotencyKey) ? OutboxState.Requested : OutboxState.PendingIdempotency
        };
        ...
        if (!string.IsNullOrEmpty(idempotencyKey))
        {
            ...
            if (!added)
            {
                ...
                outboxMessage.ProcessError = "Another request owns this idempotency key.";
            ...
            outboxMessage.OutboxState = OutboxState.Requested;

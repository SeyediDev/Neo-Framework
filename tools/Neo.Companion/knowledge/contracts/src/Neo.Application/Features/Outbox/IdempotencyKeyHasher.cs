...
{
    /// <summary>Versioned, unambiguous full hash for new storage. Legacy hashes remain readable by existing Mongo deployments.</summary>
    public static string CreateStorageKey(string tenantId, string idempotencyKey)
    {
        ArgumentNullException.ThrowIfNull(tenantId);
        ArgumentNullException.ThrowIfNull(idempotencyKey);
        var bytes = Encoding.UTF8.GetBytes(System.Text.Json.JsonSerializer.Serialize(new[] { tenantId, idempotencyKey }));
        return "v2:" + Convert.ToHexString(SHA256.HashData(bytes)).ToLowerInvariant();
    }
